...
def render():
    ...
    # Get current user's mock ID based on their login name (Intern_1 or Intern_2)
    username = st.session_state.user_display_name
    ...
    if my_summary.empty:
        st.error(f"Could not find data for {username}. Ensure your login display name matches an intern name in the database (e.g., Intern_1).")
        st.write("Showing fallback data for Intern_1 instead.")
        username = "Intern_1"
        my_summary = summary_df[summary_df['Intern_ID'] == username]
