...
# prefer DATABASE_URL if provided, otherwise fall back to explicit config
...
def load_data(data_dir=None): # data_dir kept for compatibility but not used now
    print("--> Connecting to PostgreSQL and loading data...")
    
    conn = get_connection()
    
    # pull activity logs from fact_activity + dim tables
    activity_query = """
        SELECT 
    ...
    # pull assignment progress from fact_learning_progress + dim tables
    # map course name to short tech label
    assignment_query = """
        SELECT
            di.intern_name AS intern_id,
            CASE dc.course_name
    ...
    conn.close()
    print(f"Loaded {len(activity_df)} activity rows and {len(assignments_df)} assignment rows from DB!")
